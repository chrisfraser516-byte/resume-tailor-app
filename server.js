...
function handleConfigRequest(req, res) {
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    sendJson(res, 500, {
      error: 'Supabase is not configured on the server. Set SUPABASE_URL and SUPABASE_ANON_KEY environment variables.'
    });
    return;
  }
  // The anon key is specifically designed to be safe for browser/public use —
  // Supabase's Row Level Security rules are what actually protect the data,
  // not secrecy of this key. The service_role key (far more powerful) is never sent here.
  sendJson(res, 200, {
...
server.listen(PORT, () => {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    console.log('  ⚠️  WARNING: SUPABASE_URL and/or SUPABASE_ANON_KEY are not set. Accounts/login will not work until these are set.\n');
  }
});
